...
pub fn install_lifecycle_observers() {
    unsafe {
        markerup_ios_install_lifecycle_observers();
    }
}
...
extern "C" fn picker_callback(
    ...
    let result = if path.is_null() {
        ...
        let bookmark = if bookmark.is_null() {
            ...
            unsafe { std::slice::from_raw_parts(bookmark, bookmark_len) }.to_vec()
        };
...
pub fn resolve_bookmark(bookmark: &[u8]) -> Result<WorkspaceSelection, String> {
    ...
    let resolved =
        unsafe { markerup_ios_resolve_bookmark(bookmark.as_ptr(), bookmark.len(), &mut path) };
    if !resolved || path.is_null() {
        return Err("The saved workspace permission is no longer valid".to_string());
    }
    let path_text = unsafe { CStr::from_ptr(path) }
        .to_string_lossy()
        .into_owned();
...
pub fn read_file(path: &std::path::Path) -> Result<Vec<u8>, String> {
    ...
    if !ok || data.is_null() {
        return Err("coordinated file read failed".to_string());
    }
...
pub fn write_file(path: &std::path::Path, contents: &[u8]) -> Result<(), String> {
    ...
    ok.then_some(())
        .ok_or_else(|| "coordinated file write failed".to_string())
}
...
pub fn list_entries(path: &std::path::Path) -> Result<Vec<u8>, String> {
    ...
    if !ok || data.is_null() {
        return Err("coordinated directory enumeration failed".to_string());
    }
    let bytes = unsafe { std::slice::from_raw_parts(data, len) }.to_vec();
    unsafe { markerup_ios_free_data(data, len) };
    Ok(bytes)
...
pub fn diagnostics() -> String {
    ...
    if pointer.is_null() {
        return "No diagnostic report available.".to_string();
    }
    let report = unsafe { CStr::from_ptr(pointer) }
        .to_string_lossy()
        .into_owned();
    unsafe {
        markerup_ios_free_string(pointer);
    }
    report
}
